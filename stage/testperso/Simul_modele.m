...
Ts=0.1;                % Pas de temps en seconde
tsim=100;                % Temps de simulation
t = 0:Ts:tsim-Ts;       % Vecteur de temps
i=1;

eps = 50; % variation d'altitude autour du mode maintien
h(1) = 200; % altitude initiale
...
n = 5; % passage de 0.1 s à 0.5s d'échantillonage

% sys_hold
sys_holdk10 = sys_hold;

% Xk+1 = ...
sys_holdk10.a = sys_hold.a^n;
b = sys_hold.a^0;
for k = 1:n-1
    b = b + sys_hold.a^k;
end
sys_holdk10.b = b*sys_hold.b;

% Yk+1 = ...
sys_holdk10.c = sys_hold.c*sys_hold.a^n;
d = sys_hold.a^0;
for k = 1:n-1
    d = d + sys_hold.a^k;
end
...
d = d + sys_hold.d;
    ...
    % sys_climb
    sys_climbk10 = sys_climb;

    % Xk+1 = ...
    sys_climbk10.a = sys_climb.a^n;
    b = sys_climb.a^0;
    for k = 1:n-1
        b = b + sys_climb.a^k;
    end
    sys_climbk10.b = b*sys_climb.b;

    % Yk+1 = ...
    sys_climbk10.c = sys_climb.c*sys_climb.a^n;
    d = sys_climb.a^0;
    for k = 1:n-1
        d = d + sys_climb.a^k;
    end
...
% sys_descent
sys_descentk10 = sys_descent;

% Xk+1 = ...
sys_descentk10.a = sys_descent.a^n;
b = sys_descent.a^0;
for k = 1:n-1
    b = b + sys_descent.a^k;
end
sys_descentk10.b = b*sys_descent.b;

% Yk+1 = ...
sys_descentk10.c = sys_descent.c*sys_descent.a^n;
d = sys_descent.a^0;
for k = 1:n-1
    d = d + sys_descent.a^k;
end
...
% HOLD EP = 0 :
    [P0 x0 y0] = domaineLyap(sys_holdk10,'discret'); % P0 = {P_48 ; P_45 ; P_58} pour le mode HOLD
%     [P0k10 x0k10 y0k10] = domaineLyap(sys_holdk10,'discret'); % P0 = {P_48 ; P_45 ; P_58} pour le mode HOLD

% CLIMB EP = 1 : 
    [P1 x1 y1] = domaineLyap(sys_climbk10,'discret');
% DESCENT EP = -1 : 
    [P2 x2 y2] = domaineLyap(sys_descentk10,'discret');
...
clc
    ...
    disp(['FloatVar[][] ',variableTemp{1},' = new FloatVar[step-1][',int2str(nTemp),'];'])
    disp(['for(int j = 0 ; j < step-1 ; j++)'])
...
%             disp(['    store.impose( new PeqQ(vars[i][',int2str(7+k),'],',variableTemp{1},'[i-1][',int2str((k)*(nEtat+nEntre) + (k-1)*(nEtat+nEntre) + (nEtat+nEntre) -1),']) );'])     
        ...
        for k = 1:nSortie % Calcul de Yk = C*Xk + D*Uk
            ...
            for j = 1:nEtat % pour toutes les colonnes de C             
                ...
                disp(['    store.impose( new PmulCeqR(vars[i-1][',int2str(7+j),'],',num2str(C(k,j)),',',variableTemp{1},'[i-1][',int2str(ind),']) );'])
            end
            ...
            for j = 1:nEntre             
                ...
                disp(['    store.impose( new PmulCeqR(vars[i-1][',int2str(j-1),'],',num2str(D(k,j)),',',variableTemp{1},'[i-1][',int2str(ind),']) );'])            
            end

            disp('    // calcul des sommes (c11*x1k + c12*x2k ...)')
            ind = ((k)*(nEtat+nEntre) + (k-1)*(nEtat+nEntre) + 1) + nTemp1 -1;
            disp(['    store.impose( new PeqQ(',variableTemp{1},'[i-1][',int2str(ind),'],',variableTemp{1},'[i-1][',int2str(((k-1)*(nEtat+nEntre)*2 + 1) + nTemp1 -1),']) );'])            
            for j = 2:(nEtat+nEntre)            
                %disp(((k)*(nEtat+nEntre) + (k-1)*(nEtat+nEntre) + j) + nTemp1)
                ind = ((k)*(nEtat+nEntre) + (k-1)*(nEtat+nEntre) + j) + nTemp1 -1;
                disp(['    store.impose( new PplusQeqR(',variableTemp{1},'[i-1][',int2str(ind-1),'],',variableTemp{1},'[i-1][',int2str(((k-1)*(nEtat+nEntre)*2 + j) + nTemp1 -1),'],',variableTemp{1},'[i-1][',int2str(ind),']) );'])            
            end

%             disp(['    // vars[i][',int2str(1+k),'] = C(1,:)*x(k) + D(1,:)*u(k)'])
            disp(['    // m(i-1) = ',int2str(m),' => vars[i][',int2str(1+k),']'])
            disp(['    store.impose(new IfThen(new PeqC(vars[i-1][5],',int2str(m),') , new PeqQ(vars[i][',int2str(1+k),'],',variableTemp{1},'[i-1][',int2str(((k)*(nEtat+nEntre) + (k-1)*(nEtat+nEntre) + (nEtat+nEntre) ) + nTemp1 -1),']) ) );'])            
