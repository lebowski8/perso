...
tsim=200*Ts;                % Temps de simulation
...
u = [ 235*ones(1,length(t)); 1000*ones(1,length(t))];
...
clc
    ...
    C = sys.C;
    D = sys.D;
    nTemp1 = ((nEtat+nEntre)*2)*(nEtat);
    nTemp2 = ((nEtat+nEntre)*2)*(nSortie);
    ...
    for k = 1:nEtat % Calcul de Xk+1 = A*Xk + B*Uk
        ...
        for j = 1:nEtat % pour toutes les colonnes de A            
            ...
            disp(['    store.impose( new PmulCeqR(vars[i][',int2str(7+j),'],',num2str(A(k,j)),',',variableTemp{1},'[i][',int2str(ind),']) );'])
        end
        ...
        for j = 1:nEntre           
            ...
            disp(['    store.impose( new PmulCeqR(vars[i][',int2str(j-1),'],',num2str(B(k,j)),',',variableTemp{1},'[i][',int2str(ind),']) );'])            
        end

        disp('    // calcul des sommes (a11*x1k + a12*x2k ...)')
        ind = (k)*(nEtat+nEntre) + (k-1)*(nEtat+nEntre) + 1 -1;
        disp(['    store.impose( new PeqQ(',variableTemp{1},'[i][',int2str(ind),'],',variableTemp{1},'[i][',int2str((k-1)*(nEtat+nEntre)*2 + 1 -1),']) );'])            
        for j = 2:(nEtat+nEntre)            
            %disp((k)*(nEtat+nEntre) + (k-1)*(nEtat+nEntre) + j) 
            ind = (k)*(nEtat+nEntre) + (k-1)*(nEtat+nEntre) + j -1;
            disp(['    store.impose( new PplusQeqR(',variableTemp{1},'[i][',int2str(ind-1),'],',variableTemp{1},'[i][',int2str((k-1)*(nEtat+nEntre)*2 + j -1),'],',variableTemp{1},'[i][',int2str(ind),']) );'])            
        end

%             disp(['    // vars[i][',int2str(7+k),'] = A(1,:)*x(k) + B(1,:)*u(k)'])
        disp(['    // Es(i) = ',int2str(m),' => vars[i+1][',int2str(7+k),']'])
%             disp(['    store.impose( new PeqQ(vars[i][',int2str(7+k),'],',variableTemp{1},'[i-1][',int2str((k)*(nEtat+nEntre) + (k-1)*(nEtat+nEntre) + (nEtat+nEntre) -1),']) );'])     
        disp(['    store.impose(new IfThen(new XeqC(varsInt[i][1],',int2str(m),') , new PeqQ(vars[i+1][',int2str(7+k),'],',variableTemp{1},'[i][',int2str((k)*(nEtat+nEntre) + (k-1)*(nEtat+nEntre) + (nEtat+nEntre) -1),']) ) );'])     
    end
    
    if (m == -1) || (m == 1)
        disp(['    // Es(i) = ',int2str(m),' => vars[i+1][',int2str(7+k),']'])
        disp(['    store.impose(new IfThen(new XeqC(varsInt[i][1],',int2str(m),') , new PeqC(vars[i+1][18],0) ) );'])             
    end
    disp('}')
    
    disp(['for(int i = 0 ; i < step ; i++){'])
...
%             disp(['    // vars[i][',int2str(1+k),'] = C(1,:)*x(k) + D(1,:)*u(k)'])
        disp(['    // Ep(i) = ',int2str(m),' => vars[i][',int2str(1+k),']'])
        disp(['    store.impose(new IfThen(new XeqC(varsInt[i][0],',int2str(m),') , new PeqQ(vars[i][',int2str(1+k),'],',variableTemp{1},'[i][',int2str(((k)*(nEtat+nEntre) + (k-1)*(nEtat+nEntre) + (nEtat+nEntre) ) + nTemp1 -1),']) ) );'])            
