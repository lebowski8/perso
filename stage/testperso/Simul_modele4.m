...
%             disp(['    store.impose( new PeqQ(vars[i][',int2str(7+k),'],',variableTemp{1},'[i-1][',int2str((k)*(nEtat+nEntre) + (k-1)*(nEtat+nEntre) + (nEtat+nEntre) -1),']) );'])     
        disp(['    store.impose(new IfThen(new PeqC(vars[i][20],',int2str(m),') , new PeqQ(vars[i+1][',int2str(7+k),'],',variableTemp{1},'[i][',int2str((k)*(nEtat+nEntre) + (k-1)*(nEtat+nEntre) + (nEtat+nEntre) -1),']) ) );'])     
    end
    
    
    if (m == -1) || (m == 1)
        disp(['    // Es(i) = ',int2str(m),' => vars[i+1][',int2str(7+k),']'])
        disp(['    store.impose(new IfThen(new PeqC(vars[i][20],',int2str(m),') , new PeqC(vars[i+1][18],0) ) );'])             
    end
    disp('}')
