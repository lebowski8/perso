...
% u = [ 235*ones(1,length(t)); 1500*ones(1,length(t))];

u = [235*ones(1,length(t)) ; 1500 1500 1500 1500 1500 1500 1125]; % test planif
...
%             disp(['    // vars[i][',int2str(7+k),'] = A(1,:)*x(k) + B(1,:)*u(k)'])
        disp(['    // Es(i-1) = ',int2str(m),' => vars[i][',int2str(7+k),']'])
%             disp(['    store.impose( new PeqQ(vars[i][',int2str(7+k),'],',variableTemp{1},'[i-1][',int2str((k)*(nEtat+nEntre) + (k-1)*(nEtat+nEntre) + (nEtat+nEntre) -1),']) );'])     
        disp(['    store.impose(new IfThen(new PeqC(vars[i-1][20],',int2str(m),') , new PeqQ(vars[i][',int2str(7+k),'],',variableTemp{1},'[i-1][',int2str((k)*(nEtat+nEntre) + (k-1)*(nEtat+nEntre) + (nEtat+nEntre) -1),']) ) );'])     
    end
    
    
    if (m == -1) || (m == 1)
        disp(['    // Es(i-1) = ',int2str(m),' => vars[i][',int2str(7+k),']'])
        disp(['    store.impose(new IfThen(new PeqC(vars[i-1][20],',int2str(m),') , new PeqC(vars[i][18],0) ) );'])             
    end
    disp('}')
